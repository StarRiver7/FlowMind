...
def build_intern_graph():
    ...
    graph.add_conditional_edges("router_node", route_after_router, {"chat_node": "chat_node"})
...
class InternGraph:
    ...
    async def run(self, user_id, conversation_id, message, history=None, model_name="deepseek-chat"):
        state = create_initial_state(user_id=user_id, conversation_id=conversation_id, message=message, history=history, model_name=model_name)
